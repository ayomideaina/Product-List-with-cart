...
function updateButtonText(itemName) {
    const button = document.getElementById(`addToCart-${itemName.replace(/\s+/g, '-')}`);
    if (cart[itemName]) {
        button.innerHTML = `
        <div class="quantity">
            <span onclick="decreaseQuantity('${itemName}')">-</span>
            ${cart[itemName].quantity}
            <span onclick="increaseQuantity('${itemName}')">+</span>
        </div>
        `;
    } else {
        button.innerHTML = `
